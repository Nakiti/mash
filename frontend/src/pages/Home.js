...
const Home = () => {
   ...
   useEffect(() => {
      ...
      socket.on("updatePlays", (input) => {
         const data = JSON.parse(input)

         if (data.type == "MASH_DATA" ) {
            setMusic(prevMusic => 
               prevMusic?.map(item => item.id === data.mash.id ? { ...item, plays: data.mash.plays } : item)
            );
            setSports(prevSports => 
               prevSports?.map(item => item.id === data.mash.id ? { ...item, plays: data.mash.plays } : item)
            );
            setFilm(prevFilm => 
               prevFilm?.map(item => item.id === data.mash.id ? { ...item, plays: data.mash.plays } : item)
            );
         }
